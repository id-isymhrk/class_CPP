...
public:
    ...
    ArrayStack(const ArrayStack &another_stack)
    {
        ...
        _items = new std::string[another_stack.size()];

        for (int i = 0; i < another_stack.size(); ++i)
        {
            _items[i] = another_stack._items[i];
        }
    }

    ArrayStack &operator=(ArrayStack &another_stack)
    {
        if (this != &another_stack)
        {
            delete[] _items;
            _num_items = another_stack._num_items;
            _allocated_size = another_stack._allocated_size;
    ...
    // move
    ArrayStack(ArrayStack &&another_stack)
    {
        delete[] _items;
        _num_items = 0;
        _allocated_size = 0;
        ...
        // delete[] another_stack._items;
        another_stack._num_items = 0;
        another_stack._allocated_size = 0;
    }
